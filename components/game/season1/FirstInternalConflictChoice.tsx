...
import { firstInternalConflictEvent } from "./firstInternalConflict";

type FirstInternalConflictChoiceProps = {
  gameState: GameState;
  onChoose: (event: GameEvent, option: EventOption) => void;
};

export function FirstInternalConflictChoice({
  gameState,
  onChoose,
}: FirstInternalConflictChoiceProps) {
  const event = useMemo(
    () =>
      gameEngine.getEventWithVisibleOptions(
        gameState,
        firstInternalConflictEvent,
      ),
    [gameState],
  );
