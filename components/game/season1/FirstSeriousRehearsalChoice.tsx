...
import { firstSeriousRehearsalEvent } from "./firstSeriousRehearsal";

type FirstSeriousRehearsalChoiceProps = {
  gameState: GameState;
  onChoose: (event: GameEvent, option: EventOption) => void;
};

export function FirstSeriousRehearsalChoice({
  gameState,
  onChoose,
}: FirstSeriousRehearsalChoiceProps) {
  const event = useMemo(
    () =>
      gameEngine.getEventWithVisibleOptions(gameState, firstSeriousRehearsalEvent),
    [gameState],
  );
